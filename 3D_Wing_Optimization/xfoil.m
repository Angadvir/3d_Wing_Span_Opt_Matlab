function [pol,foil] = xfoil(coord,Re,cl,varargin)
% Modified Mar. 2015 by Sean Wu & Sheida Hossenini (UC Davis MAE Dept.)
% to run xfoil at constant cl and suppress xfoil plot output window. 
...
%    cl: Angle-of-attack, can be a vector for an cl polar
...
%
% Outputs:
%  polar: structure with the polar coefficients (cl,CL,CD,CDp,CM,
%          Top_Xtr,Bot_Xtr) 
...
%
% If the output array does not have all cls in it, that indicates a convergence failure in Xfoil.
...
%    % Plot the results
%    figure; 
%    plot(pol.cl,pol.CL); xlabel('cl [\circ]'); ylabel('C_L'); title(pol.name);  
%    figure; subplot(3,1,[1 2]);
%    plot(foil(1).xcp(:,end),foil(1).cp(:,end)); xlabel('x');
%    ylabel('C_p'); title(sprintf('%s @ %g\\circ',pol.name,foil(1).cl(end))); 
...
Ncl = length(cl); % Number of cls swept
% default foil name
foil_name = mfilename; 
...
%   fprintf(fid,'mach %g\n',Mach);
  ...
  % Polar accumulation 
  fprintf(fid,'pacc\n\n\n');
%  Xfoil cl calculations
  [file_dump, file_cpwr] = deal(cell(1,Ncl)); % Preallocate cell arrays
  
  for ii = 1:Ncl
    % Individual output filenames
    file_dump{ii} = sprintf('%s_a%06.3f_dump.dat',fname,cl(ii));
    file_cpwr{ii} = sprintf('%s_a%06.3f_cpwr.dat',fname,cl(ii));
    % Commands
    fprintf(fid,'cl %g\n',cl(ii));
    fprintf(fid,'dump %s\n',file_dump{ii});
    fprintf(fid,'cpwr %s\n',file_cpwr{ii});
  ...
  ind = 1;
% Note that 
foil.cl = zeros(1,Ncl); % Preallocate cls
  ...
  for ii = 1:Ncl
    ...
    if (fid<=0),
      ...
      if ii == 1 % Use first run to determine number of panels (so that NACA airfoils work without vector input)
         ...
         % Preallocate Outputs
         [foil.s, foil.x, foil.y, foil.UeVinf, foil.Dstar, foil.Theta, foil.Cf, foil.H] = deal(zeros(Npanel,Ncl));  
      end
    ...
    if (fid<=0),
      ...
      if ii == 1 % Use first run to determine number of panels (so that NACA airfoils work without vector input)
         ...
         % Preallocate Outputs
         [foil.xcp, foil.cp] = deal(zeros(NCp,Ncl));  
         foil.xcp = C{1}(:,1);
      end      
      foil.cp(:,jj) = C{3}(:,1);
    end;
...
if only <= 1% clear files for default run
  for ii=1:Ncl % Clear out the xfoil dump files not used
      delete([wd filesep file_dump{ii}]);
      delete([wd filesep file_cpwr{ii}]);
  end 
end
  ...
  %  
  %   cl    CL        CD       CDp       CM     Top_Xtr  Bot_Xtr
  %  ------ -------- --------- --------- -------- -------- --------
...
% P = textscan(fid, '%f%1.4f%1.5f%1.5f%1.4f%1.4f%1.4f%*s%*s%*s%*s','Delimiter',  ' ', 'MultipleDelimsAsOne', true, 'HeaderLines' , 6, 'ReturnOnError', false);   
fclose(fid);
%     delete([wd filesep file_pwrt]);
  ...
  if length(pol.cl) ~= Ncl % Check if xfoil failed to converge 
     warning('One or more cl values failed to converge. Last converged was cl = %f. Rerun with ''oper iter ##'' command.\n', pol.cl(end)) 
  end
